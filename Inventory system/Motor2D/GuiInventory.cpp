...
void GuiInventory::CheckDraggedItem(GuiItem* drag_item)
{
	...
	if (IsPlaceable(drag_item, coord, false, &extra_item))
	{
...
bool GuiInventory::IsPlaceable(GuiItem* item, iPoint& coord, bool exchange, GuiItem** extra_item)
{
	...
	if (exchange == false)
	{
		//TODO 4: fill this part of IsPlaceable() so, it returns if the item can or not be placed.
...
{
	iPoint ret(0, 0);
	//TODO 6:Fill GetCoordFromItem() function so, from a given item, returns the coordinate of the slot where the item's pivot is placed 
